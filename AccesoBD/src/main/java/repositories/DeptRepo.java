...
public class DeptRepo {
    public static BDController db = BDController.getInstance();
    ...
    private static Optional<Departamento> getDepartamentoFromResultSet(ResultSet dbResult) throws SQLException {
        return Optional.of(
                new Departamento(
                        ...
                        ProgRepo.selectProgramadorById(dbResult.getInt("id_departamento")).orElse(null),
                        ProgRepo.selectProgramadoresDeDepartamentoConId(dbResult.getInt("id_departamento"))
                ));
    }
    ...
    public static Optional<Departamento> deleteDepartamento(Departamento departamento) {
        ...
        try {
            ...
            if (db.delete(query, departamento.getId()) > 0) {
                departamento.getProgramadores().forEach(ProgRepo::deleteProgramador);
    ...
    public static Optional<Departamento> insertDepartamento(Departamento departamento) {
        ...
        try {
            ...
            returnOptional = Optional.of(departamento);
            if (result.next()) {
                departamento.getProgramadores().forEach(ProgRepo::insertProgramador);
            }
            db.close();
        } catch (SQLException e) {
            e.printStackTrace();
